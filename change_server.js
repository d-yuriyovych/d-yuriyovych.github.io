...
function startMe() { 
    ...
    // 1. ПОТОЧНИЙ СТАТУС (З ГАЛОЧКОЮ ТА СТАТУСОМ)
    Lampa.SettingsApi.addParam({
        component: 'location_redirect',
        param: { name: 'main_status', type: 'scroll' },
        ...
        onRender: function(item) {
            checkOnline(current_host, function(isOk) {
                ...
                var mark = (Lampa.Storage.get('location_server') === '-' || !Lampa.Storage.get('location_server')) ? '<span style="color:#2ecc71">✓ </span>' : '';
                item.find('.settings-param__name').html(mark + 'Поточний : <span style="color:' + color + '">' + current_friendly + status + '</span>');
            });
        }
    });
    ...
    Lampa.SettingsApi.addParam({
        component: 'location_redirect',
        param: { name: 'apply_reload', type: 'scroll' },

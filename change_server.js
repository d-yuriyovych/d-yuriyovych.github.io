...
// Функція для отримання гарної назви
...
function checkOnline(url, callback) {
    ...
    var img = new Image();
    // Використовуємо протокол поточної сторінки для уникнення блокувань
    var protocol = window.location.protocol;
    var domain = url.split('?')[0].replace(/\/$/, "");
    var testUrl = (domain.indexOf('://') === -1) ? protocol + '//' + domain : domain;
...
function startMe() { 
    ...
    Lampa.SettingsApi.addParam({ 
        ...
        onRender: function(item) {
            // Перевірка всіх серверів при відмальовуванні
            setTimeout(function() {
                var $valueField = item.find('.settings-param__value');
                var currentSelection = Lampa.Storage.get('location_server') || '-';

                Object.keys(servers).forEach(function(key) {
                    var domainToTest = (key === '-') ? current_host : key;
                    
                    checkOnline(domainToTest, function(isOk) {
                        var statusText = isOk ? ' - доступний' : ' - недоступний';
                        var color = isOk ? '#2ecc71' : '#ff4c4c';
                        
                        // Оновлюємо текст, якщо цей сервер зараз вибраний у списку
                        if ($valueField.text().indexOf(servers[key]) !== -1) {
                            $valueField.html('<span style="color:' + color + '">' + servers[key] + statusText + '</span>');
                        }
                    });
                });
            }, 10);

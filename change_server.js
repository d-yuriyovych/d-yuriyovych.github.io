(function() {
    ...
    function startMe() {
        ...
        // Редирект (оригінальна логіка)
        if (window.location.search !== '?redirect=1') {
            if (current_host !== saved_server && saved_server !== '-' && saved_server !== '') {
        ...
        // Список серверів (як у вашому першому коді)
        var servers = {
            '-': 'Поточний: ' + current_host,
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Виберіть сервер Lampa'
            },
            onChange: function(value) {
                // Викликаємо перевірку перед редиректом
                startMe();
            },
            onRender: function(item) {
                // Оновлюємо відображення статусу в самому селекторі (коли меню відкрите)
                setTimeout(function() {
                    var $value_field = item.find('.settings-param__value');
                    var current_val = Lampa.Storage.get('location_server');
                    
                    // Перевіряємо статус для кожного сервера в списку
                    Object.keys(servers).forEach(function(key) {
                        var host_to_test = (key === '-') ? current_host : key;
                        
                        checkStatus(host_to_test, function(available) {
                            var statusText = available ? ' - доступний' : ' - недоступний';
                            var color = available ? '#00ff00' : '#ff3333';
                            
                            // Якщо цей сервер зараз вибраний, фарбуємо його текст у головному полі
                            if (current_val === key || (current_val === '' && key === '-')) {
                                $value_field.html('<span style="color:' + color + '">' + servers[key] + statusText + '</span>');
                            }
                        });
                    });
                }, 50);

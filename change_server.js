...
function checkOnline(url, callback) {
    ...
    // Спершу пробуємо HTTPS, потім HTTP
        ...
        onRender: function(item) {
            item.addClass('selector selector-item').on('hover:enter click', function() {
                if (selected_target) {
                    ...
                    // КРОК 1: Жорсткий перезапис усіх ключів
                    Lampa.Storage.set('server_url', domain);
                    Lampa.Storage.set('location_server', protocol + domain);
                    localStorage.setItem('server_url', domain);
                    
                    Lampa.Noty.show('Виконується перехід...');

                    setTimeout(function(){
                        // КРОК 2: Редирект на чистий домен з визначеним протоколом
...
// Запуск перевірок при кожному відкритті меню

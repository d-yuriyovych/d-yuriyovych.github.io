...
function checkOnline(url, callback) {
    ...
    // Додаємо http:// тільки для перевірки, якщо його немає
    var testUrl = (url.indexOf('://') === -1) ? 'http://' + url : url;
    img.src = testUrl.split('?')[0].replace(/\/$/, "") + '/favicon.ico?v=' + Math.random();
...
function startMe() {
    ...
    // ОРИГІНАЛЬНИЙ РЕДІРЕКТ (БЕЗ ЗМІН)
    ...
    Lampa.SettingsApi.addParam({
        ...
        onRender: function(item) {
            // Оновлення статусів для всього списку
            setTimeout(function() {
                var $valField = item.find('.settings-param__value');
                
                Object.keys(server_values).forEach(function(key) {
                    var domain = (key === '-') ? current_host : key;
                    
                    checkOnline(domain, function(isOk) {
                        var status = isOk ? ' - доступний' : ' - недоступний';
                        var color = isOk ? '#2ecc71' : '#e74c3c';
                        
                        // Якщо цей пункт зараз відображається як обраний
                        if ($valField.text().indexOf(server_values[key]) !== -1) {
                            $valField.html('<span style="color:' + color + '">' + server_values[key] + status + '</span>');
                        }
                    });
                });
            }, 100);

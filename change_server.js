(function() {
    ...
    function checkServerStatus(url, callback) {
        if (url === '-') return callback(true); // "Поточний" завжди доступний
        ...
        // Спроба завантажити фавіконку або просто пінганути корінь
        img.src = 'https://' + url + '/favicon.ico?v=' + startTime;
    ...
    function startMe() {
        ...
        if (window.location.search !== '?redirect=1') {
            ...
            if (target && target !== '-' && target !== window.location.hostname) {
                window.location.href = 'https://' + target + '?redirect=1';
            }
        } else {
            // Очищуємо параметр після успішного переходу, щоб не було циклу
            Lampa.Storage.set('location_server', '-');
        }

        // Реєстрація компонента налаштувань
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                if (value !== '-') {
                    startMe(); // Запуск редиректу
                }
            },
            onRender: function(item) {
                // Після рендеру списку перевіряємо кожен сервер
                setTimeout(function() {
                    Object.keys(servers).forEach(function(host) {
                        if (host === '-') return;
                        
                        checkServerStatus(host, function(available) {
                            var color = available ? '#4cf34c' : '#ff4c4c';
                            var text = available ? ' (доступний)' : ' (недоступний)';
                            
                            // Знаходимо елемент у списку Lampa (це хак для зміни кольору в інтерфейсі)
                            $('div[data-name="location_server"] .settings-param__value').each(function() {
                                var $val = $(this);
                                if ($val.text().indexOf(servers[host]) !== -1) {
                                    $val.html('<span style="color:' + color + '">' + servers[host] + text + '</span>');
                                }
                            });
                        });
                    });
